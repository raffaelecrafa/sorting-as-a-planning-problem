...
Since each distinct swap pair can appear at most once and there are $\binom{n}{2}$ possible pairs, we have:
\begin{equation}
    k \leq \binom{n}{2} = \frac{n(n-1)}{2}
\end{equation}
However, a much tighter bound holds for optimal plans. As we will discuss in Section 3.2, the minimum number of swaps needed is directly related to the cycle structure of the permutation, and in practice:
\begin{equation}
    k \leq n - 1
\end{equation}
